...
def main_page(request):
    ...
    if request.method=="POST":
        ...
        if form.is_valid():
         ...
         return render(request,"codefundo/alreadyreg.html")   
    count_user=track_users.objects.all()

    return render(request,"codefundo/home.html",{"form":form,"objects":count_user,} )
@login_required
...
def user_signup(request):
    ...
    if request.method=="POST":
     ...
     if auth.is_valid():
         ...
         auth.save()
         registered=True
         return HttpResponseRedirect(reverse("codefundo:user_login"))

     else :
         print("error")
    else:
        auth=Authentic()     
    return render(request,"codefundo/signup.html",{"auth":auth,"registered":registered,"form":form })        
...
def user_login(request):
    if request.method=="POST":
        ...
        if user:
            login(request,user)
            return HttpResponseRedirect(reverse("codefundo:afterlogsign"))
        else:
            return render(request,"codefundo/return_invalid_user.html")
    else :
        return render(request,"codefundo/login.html",{"form":form},)
...
def aboutus(request):
    return render(request,"codefundo/aboutus.html",{"form":form})

def alreadyreg(request):
    return render(request,"codefundo/alreadyreg.html",{"form":form})

def afterlogsign(request):
    print("asdf")
    form=fund_gov()
    #mypaste=user_details.objects.all()
    #mypaste = sorted(mypaste,key=lambda x:x.url,reverse=True)
         ...
         # mydate=request.POST.get("date",)
          if form.is_valid():
           # cur_obj = form.save(commit=False)
            ...
            form.save()
            print("manan")
            return render(request,"codefundo/thanku.html") 
          
          else:
               return render(request,"codefundo/return_valid_data.html")   
    return render(request,"codefundo/afterlogsign.html",{"form":form } )
